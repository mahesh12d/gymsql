...
class DatabaseExtractor:
    ...
    def extract_table(self, table_name: str, updated_col: Optional[str] = None, 
        ...
        try:
            if updated_col and last_sync_time:
                ...
                """
                logger.info(f"Incremental sync for {table_name} since {last_sync_time}")
            else:
...
class S3Uploader:
    ...
    def upload_dataframe(self, df: pd.DataFrame, table_name: str, 
        ...
        try:
            ...
            logger.info(f"Successfully uploaded {table_name} to s3://{self.bucket_name}/{s3_key}")
    ...
    def upload_metadata(self, metadata: Dict[str, Any], table_name: str):
        ...
        try:
            ...
            logger.info(f"Uploaded metadata to s3://{self.bucket_name}/{s3_key}")
...
def get_last_sync_time(bucket_name: str, table_name: str) -> Optional[str]:
    ...
    try:
        ...
        if 'Contents' in response and len(response['Contents']) > 0:
            latest_metadata = response['Contents'][0]['Key']
            obj = s3_client.get_object(Bucket=bucket_name, Key=latest_metadata)
            metadata = json.loads(obj['Body'].read().decode('utf-8'))
            return metadata.get('sync_end_time')
    except Exception as e:
        logger.warning(f"Could not retrieve last sync time for {table_name}: {str(e)}")
...
def lambda_handler(event, context):
    ...
    tables_to_sync = event.get('tables', list(tables_config.get('tables', {}).keys()))
    ...
    try:
        ...
        for table_name in tables_to_sync:
            try:
                table_info = tables_config.get('tables', {}).get(table_name, {})
                ...
                df = extractor.extract_table(table_name, updated_col, last_sync)
                
                if len(df) > 0:
                    upload_result = uploader.upload_dataframe(df, table_name, actual_sync_type)
