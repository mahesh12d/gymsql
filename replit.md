...
## Recent Changes
- **October 8, 2025**: Implemented 6-month data retention policy for execution_results - (1) Created data_retention.py module with cleanup functions to delete execution_results older than 6 months (180 days), (2) Added automated scheduler (scheduler.py) that runs cleanup once every 24 hours on application startup and periodically, (3) Created admin API endpoints: GET /api/admin/data-retention/stats (view storage statistics) and POST /api/admin/data-retention/cleanup (manually trigger cleanup), (4) Submission counts remain intact as they're stored in the submissions table, only detailed execution results are cleaned up, (5) This prevents unbounded database growth while preserving user statistics and submission history.
- **October 8, 2025**: Implemented worker availability detection with heartbeat mechanism - (1) Added worker heartbeat system where worker updates Redis every 15 seconds with 30-second TTL to signal it's alive, (2) Enhanced `enqueue_submission` to check worker availability before queueing: if worker alive → queue to Redis, if worker down → execute directly, (3) Implemented `execute_submission_directly` method for direct execution that still caches results in Redis for performance, (4) Updated submission API endpoint to handle "direct" execution response when no worker is available, (5) This hybrid approach prevents jobs from getting stuck when worker is down while maintaining Redis in-memory caching benefits and reducing PostgreSQL load.
- **October 7, 2025**: Implemented Redis-to-Postgres fallback mechanism for submission queue - (1) Created `fallback_submissions` table in PostgreSQL to store submissions when Redis is unavailable, (2) Enhanced `enqueue_submission` in redis_service.py with automatic fallback: tries Redis first, falls back to Postgres on failure, (3) Implemented `recover_fallback_submissions` method to move pending submissions from Postgres back to Redis when it becomes available, (4) Updated Redis worker to recover fallback submissions on startup and periodically check every 60 seconds for pending fallback submissions, (5) This ensures zero data loss - submissions are always saved even during Redis outages, and automatically recovered when Redis comes back online.
...
-   **Redis Fallback Mechanism**: When Redis becomes unavailable, submissions automatically fall back to PostgreSQL (`fallback_submissions` table) to ensure zero data loss. The worker automatically recovers pending submissions from PostgreSQL back to Redis when it becomes available again, checking on startup and every 60 seconds during operation.
