...
# Friend endpoints
@app.get("/api/friends", response_model=List[FriendWithStats])
def get_friends(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Get list of accepted friends with their statistics"""
    ...
    return [
        FriendWithStats(
            ...
            problems_solved=friend.problems_solved,
            friendship_status="accepted"
        )
        for friend in friends
    ]

@app.get("/api/friends/leaderboard", response_model=List[FriendsLeaderboardResponse])
def get_friends_leaderboard(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Get friends leaderboard sorted by problems solved"""
    from .models import Friendship
    ...
    users = db.query(User).filter(User.id.in_(all_user_ids)).order_by(desc(User.problems_solved)).all()
...
@app.post("/api/friends", response_model=FriendshipResponse)
def send_friend_request(
    ...
    existing = db.query(Friendship).filter(
        ((Friendship.user_id == current_user.id) & (Friendship.friend_id == friend_request.friend_id)) |
        ((Friendship.user_id == friend_request.friend_id) & (Friendship.friend_id == current_user.id))
    ).first()
    ...
    # Create friendship - auto-accept for now (can be changed to pending later)
