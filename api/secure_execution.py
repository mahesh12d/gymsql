...
class SecureQueryExecutor:
    ...
    async def submit_solution(self, user_id: str, problem_id: str, query: str,
        ...
        try:
            ...
            validation_result = query_validator.validate_query(
                query, sandbox.loaded_table_names)
            ...
            if not validation_result['is_valid']:
                return {
                    ...
                    'security_violations':
                    validation_result['blocked_operations'],
                    'submission_id': None
                }
    ...
    async def test_query(self,
        ...
        try:
            ...
            execution_status = ExecutionStatus.SUCCESS if query_result.get(
                'success') else ExecutionStatus.ERROR
                ...
                if problem:
                    ...
                    if problem.solution_source == 's3' and problem.s3_solution_source:
                        ...
                        test_results = await self._verify_with_s3_solution(
                            sandbox, problem, query,
                            query_result.get('results', []))
                    else:
                        # Use Neon database verification (default)
                        if problem.question:
                            expected_output = problem.question.get(
                                'expectedOutput', [])
                            if expected_output:
                                # Compare results with expected output
                                actual_results = query_result.get(
                                    'results', [])
                                test_results.append({
                                    'test_case_id':
                                    f"{problem_id}_main",
                                    'passed':
                                    actual_results == expected_output,
                                    'expected':
                                    expected_output,
                                    'actual':
                                    actual_results,
            ...
            return {
                ...
                'query_analysis':
                validation_result['parsed_query'],
                'query_result':
                query_result,  # Include actual query execution results
                'execution_status':
                execution_status.value if execution_status else 'SUCCESS'
            }
    ...
    async def _get_or_create_sandbox(self, user_id: str, problem_id: str,
        ...
        try:
            ...
            if sandbox is not None:
                logger.info(
                    f"Using existing sandbox for user {user_id}, problem {problem_id}"
                )
            ...
            # Create new sandbox
            logger.info(
                f"Creating new sandbox for user {user_id}, problem {problem_id}"
            )
            ...
            logger.error(
                f"Failed to get or create sandbox for user {user_id}, problem {problem_id}: {e}"
            )
            return None

    async def _verify_with_s3_solution(
            self, sandbox: DuckDBSandbox, problem: Problem, user_query: str,
            user_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.info(
                f"Downloading S3 solution results from s3://{s3_solution['bucket']}/{s3_solution['key']}"
            )
            ...
            try:
                ...
                logger.info(
                    f"Successfully loaded {len(expected_results)} expected result rows from S3"
                )

            except Exception as e:
                logger.error(
                    f"Failed to download S3 solution parquet file: {e}")
                return [{
                    'test_case_id':
                    f"{problem.id}_s3_solution",
                    'passed':
                    False,
                    'expected': [],
                    'actual':
                    ...
                    False,
                    'error':
                    f'Failed to download solution parquet file: {str(e)}'
                }]

            # Compare user results with official solution results
            comparison = self._compare_query_results(user_results,
                                                     expected_results)
            ...
            logger.error(
                f"S3 solution verification failed for problem {problem.id}: {e}"
            )
            return [{
                'test_case_id': f"{problem.id}_s3_solution",
                'passed': False,
    ...
    def _compare_query_results(
        ...
        try:
            ...
            actual_tuples = [
                tuple(sorted(row.items())) for row in actual_normalized
            ]
            expected_tuples = [
                tuple(sorted(row.items())) for row in expected_normalized
            ]
            ...
            for i, expected_row in enumerate(expected):
                ...
                if matches:
                    ...
                    row_comparison[
                        'differences'] = "Row not found in user results"

                comparison_result['row_comparisons'].append(row_comparison)

            # Check if all rows match
            comparison_result['matches'] = comparison_result[
                'matching_row_count'] == comparison_result['total_row_count']
    ...
    async def get_problem_schema(self, user_id: str, problem_id: str,
                                 db: Session) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            question_data = problem.question if isinstance(
                problem.question, dict) else {}
            tables = question_data.get('tables', [])

            # Enhance with database introspection if needed
            schema_info = await self._introspect_database_schema(
                sandbox.id, tables)
    ...
    async def get_user_progress(self, user_id: str,
        ...
        try:
            ...
            submission_stats = db.query(
                ...
                func.count(
                    Submission.id).filter(Submission.is_correct == True).label(
                        'correct_submissions'),
                func.avg(
                    Submission.execution_time).label('avg_execution_time'),
                func.min(Submission.execution_time).label(
                    'best_execution_time')).filter(
            ...
            difficulty_stats = db.query(
                ...
                func.count(Submission.id).filter(
                    Submission.is_correct == True).label('solved_count')).join(
                        Submission,
                        Submission.problem_id == Problem.id).filter(
                            Submission.user_id == user_id,
                            Submission.is_correct == True).group_by(
                                Problem.difficulty).all()

            # Get recent activity
            recent_submissions = db.query(Submission).filter(
                Submission.user_id == user_id).order_by(
            ...
            return {
                ...
                'statistics': {
                    ...
                    'total_submissions':
                    submission_stats.total_submissions or 0,
                    'correct_submissions':
                    submission_stats.correct_submissions or 0,
                    'success_rate':
                    ((submission_stats.correct_submissions /
                      submission_stats.total_submissions *
                      100) if submission_stats.total_submissions > 0 else 0),
                    'avg_execution_time_ms':
                    int(submission_stats.avg_execution_time or 0),
                    'best_execution_time_ms':
                    int(submission_stats.best_execution_time or 0)
                },
                'difficulty_breakdown': {
                    stat.difficulty: stat.solved_count
    ...
    async def _execute_all_test_cases(self, sandbox_id: str, problem_id: str,
                                      query: str,
        ...
        if problem.expected_hash and problem.s3_data_source:
            logger.info(
                f"Using enhanced S3 hash validation for problem {problem_id}")
        ...
        try:
            ...
            if extracted_problem_id != problem_id:
                logger.warning(
                    f"Problem ID mismatch: {extracted_problem_id} != {problem_id}"
                )
            sandbox = self.sandbox_manager.get_sandbox(user_id, problem_id)
        except ValueError:
        ...
        for test_case in test_cases:
            try:
                ...
                execution_status = ExecutionStatus.SUCCESS if result.get(
                    'success', False) else ExecutionStatus.ERROR
                ...
                if execution_status == ExecutionStatus.SUCCESS:
                    ...
                    if test_case.expected_output_source:
                        try:
                            ...
                            if s3_config.get('bucket') and s3_config.get(
                                    'key') and s3_config.get('format'):
                                logger.info(
                                    f"Fetching expected output from S3: {s3_config['bucket']}/{s3_config['key']}"
                                )
                                ...
                                cache_result = s3_service.fetch_answer_file(
                                    ...
                                    etag=getattr(s3_answer_source, 'etag',
                                                 None))

                                expected_output = cache_result.data
                                logger.info(
                                    f"Successfully fetched {len(expected_output)} rows from S3 expected output"
                                )
                        except Exception as e:
                            logger.error(
                                f"Failed to fetch S3 expected output for test case {test_case.id}: {e}"
                            )
                    ...
                    validation = test_validator.validate_test_case(
                        ...
                        expected_output,
                        ComparisonMode.
                        UNORDERED  # Allow row reordering by default
                    )
    ...
    async def _execute_s3_hash_validation(self, problem: "Problem", query: str,
                                          db: Session) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.info(
                f"Executing user query against S3 dataset: s3://{bucket}/{key}"
            )
            ...
            try:
                ...
                table_name = s3_data.get(
                    'table_name', 'dataset')  # Use configured table name
                conn.execute(
                ...
                user_hash = s3_service.generate_expected_result_hash(
                    user_results)
                ...
                logger.info(
                    f"Hash comparison - Expected: {problem.expected_hash}, User: {user_hash}, Match: {is_correct}"
                )

                # Generate feedback
                if is_correct:
                    feedback = [
                        "🎉 Perfect! Your query produces the exact expected results."
                    ]
                    score = 100.0
                else:
                ...
                return [{
                    ...
                    'user_output':
                    user_results[:5],  # Show first 5 rows for debugging
                    'expected_output':
                    sanitize_json_data(problem.preview_rows or []),
                    'output_matches':
                    is_correct
                }]
            ...
            execution_time_ms = int((time.time() - start_time) * 1000)
            logger.error(
                f"S3 hash validation failed for problem {problem.id}: {e}")
    ...
    async def _execute_test_cases(
        ...
        query_filter = db.query(TestCase).filter(
        ...
        return await self._execute_specific_test_cases(sandbox_id, query,
                                                       test_cases)
    ...
    async def _execute_specific_test_cases(
        ...
        for test_case in test_cases:
            try:
                ...
                if execution_status == ExecutionStatus.SUCCESS:
                    validation = test_validator.validate_test_case(
                        result.get('result', []), test_case.expected_output,
    ...
    def _generate_test_feedback(
        ...
        if passed == total:
            feedback.append(
                "🎉 All visible test cases pass! Your solution looks correct.")
        elif passed > 0:
            feedback.append(
                f"{passed}/{total} test cases pass. Review the failing cases below."
            )
        else:
            feedback.append(
                "❌ No test cases pass. Check your query logic carefully.")

        # Add specific feedback from failed tests
        for result in test_results:
            if not result['is_correct']:
                feedback.append(
                    f"❌ {result['test_case_name']}: {'; '.join(result['feedback'])}"
                )
    ...
    async def _introspect_database_schema(
            self, sandbox_id: str,
            table_definitions: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _calculate_user_streak(self, user_id: str,
                                     db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                max_streak,
                'last_correct_date':
                last_correct_date.isoformat() if last_correct_date else None
            }
    ...
    async def _update_user_progress(self, user_id: str, problem_id: str,
                                    db: Session) -> None:
        ...
        try:
            ...
            if user:
                ...
                current_submission = db.query(Submission).filter(
                    ...
                    Submission.is_correct == True).order_by(
                ...
                existing_correct_count = db.query(Submission).filter(
                    ...
                    if current_submission else None).count()
